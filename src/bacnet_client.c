...
bool php_bacnet_client_queue_pdu(php_bacnet_client *client, const BACNET_ADDRESS *source,
	...
	if (!client || !source || !pdu || pdu_len == 0 || pdu_len > MAX_APDU + MAX_NPDU ||
		client->pending_count >= PHP_BACNET_PENDING_PDU_CAPACITY) {
		return false;
	}

	uint8_t index =
		(uint8_t)((client->pending_head + client->pending_count) % PHP_BACNET_PENDING_PDU_CAPACITY);
	php_bacnet_pending_pdu *pending = &client->pending_pdus[index];
	memcpy(&pending->source, source, sizeof(BACNET_ADDRESS));
	memcpy(pending->data, pdu, pdu_len);
	pending->length = pdu_len;
...
bool php_bacnet_client_pop_pdu(php_bacnet_client *client, BACNET_ADDRESS *source, uint8_t *pdu,
							   uint16_t *pdu_len) {
	if (!client || !source || !pdu || !pdu_len || client->pending_count == 0) {
		return false;
	}

	php_bacnet_pending_pdu *pending = &client->pending_pdus[client->pending_head];
	memcpy(source, &pending->source, sizeof(BACNET_ADDRESS));
	memcpy(pdu, pending->data, pending->length);
	*pdu_len = pending->length;
	client->pending_head = (uint8_t)((client->pending_head + 1) % PHP_BACNET_PENDING_PDU_CAPACITY);
	client->pending_count--;
	return true;
}
...
int php_bacnet_broadcast_and_collect(php_bacnet_client *client, uint8_t *request_apdu,
	...
	while (php_bacnet_ms_now() < deadline && count < PHP_BACNET_MAX_COLLECTED_DEVICES) {
		uint64_t remaining = deadline - php_bacnet_ms_now();
		if (remaining == 0)
			break;
		...
		uint16_t pdu_len = bip_receive(&src, pdu, (uint16_t)sizeof(pdu), (unsigned)remaining);
		...
		if (npdu_hdr.network_layer_message)
			continue;
		...
		int rc =
			iam_decode_service_request(apdu + 2, &device_id, &max_apdu, &segmentation, &vendor_id);
		if (rc < 0)
			continue;

		if (client->ignore_device_id && device_id == client->local_device_id) {
			continue;
		}
...
int php_bacnet_send_and_wait(php_bacnet_client *client, BACNET_ADDRESS *dest, uint8_t *request_apdu,
	...
	while (php_bacnet_ms_now() < deadline) {
		uint64_t remaining = deadline - php_bacnet_ms_now();
		if (remaining == 0)
			break;
...
void php_bacnet_address_to_ipport(const BACNET_ADDRESS *addr, char *ipbuf, size_t ipbuflen,
								  uint16_t *port) {
	if (addr->mac_len >= 6) {
		snprintf(ipbuf, ipbuflen, "%u.%u.%u.%u", addr->mac[0], addr->mac[1], addr->mac[2],
				 addr->mac[3]);
