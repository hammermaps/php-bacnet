...
static const struct {
	...
	const char *name;
} s_ot_map[] = {{OBJECT_ANALOG_INPUT, "ANALOG_INPUT"},
...
static bool php_bacnet_find_ot_case(BACNET_OBJECT_TYPE type, zval *out) {
	ZVAL_UNDEF(out);
...
void php_bacnet_oid_from_c(BACNET_OBJECT_TYPE type, uint32_t instance, zval *zv) {
	...
	zval type_zv;
	if (php_bacnet_find_ot_case(type, &type_zv)) {
		ZVAL_COPY_VALUE(&obj->type_zval, &type_zv);
		zend_update_property(bacnet_ce_object_identifier, Z_OBJ_P(zv), "type", strlen("type"),
							 &type_zv);
	} else {
		ZVAL_UNDEF(&obj->type_zval);
		zend_update_property_null(bacnet_ce_object_identifier, Z_OBJ_P(zv), "type", strlen("type"));
	}
	zend_update_property_long(bacnet_ce_object_identifier, Z_OBJ_P(zv), "instance",
							  strlen("instance"), (zend_long)instance);
}
...
void php_bacnet_bitstring_new(BACNET_BIT_STRING *bs, zval *zv) {
	...
	memcpy(obj->value, bs->value, obj->bits_used > 0 ? (size_t)((obj->bits_used + 7) / 8) : 0);
}
...
void php_bacnet_date_new(BACNET_DATE *d, zval *zv) {
	...
	zend_update_property_long(bacnet_ce_date, Z_OBJ_P(zv), "year", strlen("year"),
							  (zend_long)d->year);
	zend_update_property_long(bacnet_ce_date, Z_OBJ_P(zv), "month", strlen("month"),
							  (zend_long)d->month);
	zend_update_property_long(bacnet_ce_date, Z_OBJ_P(zv), "day", strlen("day"), (zend_long)d->day);
	zend_update_property_long(bacnet_ce_date, Z_OBJ_P(zv), "weekday", strlen("weekday"),
							  (zend_long)d->wday);
}
...
void php_bacnet_time_new(BACNET_TIME *t, zval *zv) {
	...
	zend_update_property_long(bacnet_ce_time, Z_OBJ_P(zv), "hour", strlen("hour"),
							  (zend_long)t->hour);
	zend_update_property_long(bacnet_ce_time, Z_OBJ_P(zv), "minute", strlen("minute"),
							  (zend_long)t->min);
	zend_update_property_long(bacnet_ce_time, Z_OBJ_P(zv), "second", strlen("second"),
							  (zend_long)t->sec);
	zend_update_property_long(bacnet_ce_time, Z_OBJ_P(zv), "hundredths", strlen("hundredths"),
							  (zend_long)t->hundredths);
}
...
void bacapp_value_to_zval(BACNET_APPLICATION_DATA_VALUE *val, zval *zv) {
	...
	case BACNET_APPLICATION_TAG_OBJECT_ID:
		php_bacnet_oid_from_c(val->type.Object_Id.type, val->type.Object_Id.instance, zv);
		break;
...
void bacapp_values_to_zval(uint8_t *apdu, unsigned apdu_len, zval *out) {
	...
	while (offset < apdu_len) {
		...
		memset(&val, 0, sizeof(val));
		int n = bacapp_decode_application_data(apdu + offset, apdu_len - offset, &val);
		if (n <= 0)
			break;
		offset += (unsigned)n;
