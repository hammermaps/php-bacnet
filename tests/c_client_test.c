...
int main(int argc, char *argv[]) {
	...
	printf("  interface : %s\n", iface);
	printf("  timeout   : %u ms\n", timeout_ms);
	printf("  device cap: %d\n\n", PHP_BACNET_MAX_COLLECTED_DEVICES);

	char *err_msg = NULL;
	php_bacnet_client *client = php_bacnet_client_create(iface, PHP_BACNET_DEFAULT_PORT, &err_msg);
	if (!client) {
	...
	int count =
		php_bacnet_broadcast_and_collect(client, apdu, (uint16_t)apdu_len, entries, timeout_ms);
	...
	for (int i = 0; i < count; i++) {
		...
		printf("  [%d] device_id=%u  addr=%s:%u  max_apdu=%u  vendor=%u\n", i, entries[i].device_id,
			   ipbuf, (unsigned)port, entries[i].max_apdu, entries[i].vendor_id);
	}
